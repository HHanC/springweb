...
public class BoardService {
    ...
    public boolean save(BoardDto boardDto){
        ...
        // 1. 세션호출 [시큐리티 사용시 -> 세션x -> dlswmdtptus(UserDetails vs DefaultOAuth2User)]
        ...
        // 2. 인증 정보 가져오기
        Object principal = authentication.getPrincipal(); // Principal : 인증정보
        // 3. 일반회원 : UserDetails , oauth회원 : DefaultOAuth2User
            // java문법 : 자식객체 instanceof 부모클래스명 : 상속 확인 키워드
        String mid = null;
        if(principal instanceof UserDetails){ // 인증정보의 타입이 UserDetails 이면 [일반회원 검증]
            mid = ((UserDetails) principal).getUsername(); // 인증 정보에서 mid 호출
            System.out.println("일반 회원으로 글쓰기!!!" + principal.toString());
        }else if(principal instanceof DefaultOAuth2User){ // 인증 정보의 타입이 DefaultOAuth2User 이면 [oauth2회원 검증]
            System.out.println("oauth2 회원으로 글쓰기!!!" + principal.toString());
            // 회원정보 요청키를 이용한 구분 짓기
             Map<String,Object> map = ((DefaultOAuth2User) principal).getAttributes();
            if(map.get("response") != null){  // 1. 네이버일경우 [Attributes에 response 이라는 키가 존재하면]
        ...
        if(mid != null){ // 로그인이 되어있으면 : Optional클래스[null 저장]
            // 2. 로그인된 회원의 엔티티 찾기
