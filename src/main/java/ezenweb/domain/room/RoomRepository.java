...
public interface RoomRepository extends JpaRepository<RoomEntity,Integer> {
    ...
    List<RoomEntity> search(@Param("trans") int trans , @Param("minprice") int minprice , @Param("maxprice") int maxprice);

    // 2. 가격 검색
    @Query(value = "select * from room where rprice > :minprice and rprice < :maxprice", nativeQuery = true)
    List<RoomEntity> search(@Param("minprice") int minprice , @Param("maxprice") int maxprice);

    // 오버라이딩[재정의] vs 오버로딩 [동일한 메소드명 의 인수/타입이 다를때]
